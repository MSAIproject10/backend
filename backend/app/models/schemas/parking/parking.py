...
from sqlalchemy import Column, Integer, String, Unicode, UnicodeText, Float, Boolean
...
class Parking(Base):
    ...
    external_id = Column(Unicode(50))  # 주차장 외부 ID
    parking_name = Column(Unicode(100))  # 한글 주차장명
    address = Column(UnicodeText)        # 한글 주소
    parking_type = Column(Unicode(20))   # 노상/노외
    phone_number = Column(UnicodeText)   # 전화번호 (일부 빈 값 존재)

    latitude = Column(Float, nullable=True)   # 위도
    longitude = Column(Float, nullable=True)  # 경도
    
    operation_type = Column(Unicode(50)) # 운영구분명(시간제 + 버스전용 주차장)
